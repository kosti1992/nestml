...
def functional_shapes_to_odes(neuron, transformed_shapes):
    ...
    for declaration in ASTHelper.get_equations_block_from_neuron(neuron).get_declarations():
    ...
    for declaration in shapes_to_delete:
        ASTHelper.get_equations_block_from_neuron(neuron).get_declarations().remove(declaration)
